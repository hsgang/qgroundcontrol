...
bool LinkManager::createConnectedLink(SharedLinkConfigurationPtr& config, bool isPX4Flow)
{
    ...
    if (link) {
        ...
        connect(link.get(), &LinkInterface::bytesReceived,       _siyiSDKManager,     &SiyiSDKManager::receivedLinkStatus);
        ...
        if (!link->_connect()) {
            ...
            config->linkChanged();
            config->setLink(nullptr); // weak_ptr is cleaned up
            return false;
        }
...
void LinkManager::_linkDisconnected(void)
{
    ...
    disconnect(link, &LinkInterface::bytesReceived,       _siyiSDKManager,     &SiyiSDKManager::receivedLinkStatus);
