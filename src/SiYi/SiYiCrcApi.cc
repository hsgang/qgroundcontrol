...
uint16_t CRC16_cal(quint8 *ptr, quint32 len, quint16 crc_init)
{
...
SiYiCrcApi::SiYiCrcApi(QObject *parent)
    :QObject (parent)
{

}

quint16 SiYiCrcApi::calculateCrc16(const QByteArray &bytes)
{
    QByteArray input = bytes;
    uint16_t ret = CRC16_cal(
        reinterpret_cast<uint8_t*>(input.data()), input.length(), 0);
    return ret;
}

quint32 SiYiCrcApi::calculateCrc32(const QByteArray &bytes)
{
    QByteArray input = bytes;
