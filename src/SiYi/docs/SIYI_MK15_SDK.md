...
> Source: *MK15 SDK*, §4.8 "SIYI Datalink SDK" (pp. 106–116 of 153).
> Transcribed from `mk15_sdk.pdf`. Example frames CRC-verified (CRC-16/XMODEM, see [§4](#4-crc16)).
>
...
> over TCP `192.168.144.12:5864` (a different, legacy framing — see the note in [§3](#3-communication-interface)).
...
- **Note**: this exact `int32 × 9` layout matches [`SiYiTransmitter::HeartbeatAckContext`](../SiYiTransmitter.h).
...
- **ACK**: `uint32 rc_version`, `rf_version`, `ground_version`, `sky_version`.
  ...
  (e.g. `00 03 05 68` → product `0x68`, version `5.3.0`).
...
> MK15 has **no** `0x4E` (multi-device) or `0x4F` (system status) commands — those are UniRC 7 only.
...
> `int32 × 9` link-status payload as `0x44` above.
