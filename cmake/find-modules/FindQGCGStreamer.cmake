...
function(find_gstreamer_component component pkgconfig_name)
    ...
    if(NOT TARGET ${target})
        ...
        if(TARGET PkgConfig::PC_GSTREAMER_${upper})
            # On Android, strip x86_64 libunwind.so from pkg-config dependencies
            if(ANDROID)
                get_target_property(_pc_comp_libs PkgConfig::PC_GSTREAMER_${upper} INTERFACE_LINK_LIBRARIES)
                if(_pc_comp_libs)
                    list(FILTER _pc_comp_libs EXCLUDE REGEX "[Uu]nwind")
                    set_target_properties(PkgConfig::PC_GSTREAMER_${upper} PROPERTIES INTERFACE_LINK_LIBRARIES "${_pc_comp_libs}")
                endif()
            endif()
...
if(QGCGStreamer_FOUND AND NOT TARGET GStreamer::GStreamer)
    ...
    if(GStreamer_USE_STATIC_LIBS)
        ...
        # On Android, NDK's prebuilt x86_64 libunwind.so leaks into the link line
        # via pkg-config transitive dependencies. Strip it from all PkgConfig plugin
        # targets and from GStreamer::Plugins itself.
        if(ANDROID)
            foreach(plugin IN LISTS GSTREAMER_PLUGINS)
                if(TARGET PkgConfig::GST_PLUGIN_${plugin})
                    get_target_property(_pk_libs PkgConfig::GST_PLUGIN_${plugin} INTERFACE_LINK_LIBRARIES)
                    if(_pk_libs)
                        list(FILTER _pk_libs EXCLUDE REGEX "[Uu]nwind")
                        set_target_properties(PkgConfig::GST_PLUGIN_${plugin} PROPERTIES INTERFACE_LINK_LIBRARIES "${_pk_libs}")
                    endif()
                endif()
            endforeach()
            # Also clean the core GStreamer PkgConfig target
            if(TARGET PkgConfig::PC_GSTREAMER)
                get_target_property(_pc_gst_libs PkgConfig::PC_GSTREAMER INTERFACE_LINK_LIBRARIES)
                if(_pc_gst_libs)
                    list(FILTER _pc_gst_libs EXCLUDE REGEX "[Uu]nwind")
                    set_target_properties(PkgConfig::PC_GSTREAMER PROPERTIES INTERFACE_LINK_LIBRARIES "${_pc_gst_libs}")
                endif()
            endif()
        endif()
