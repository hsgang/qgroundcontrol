...
if [[ $# -eq 0 ]]; then
  echo 'create_linux_missioncontrol_appimage.sh QGC_SRC_DIR QGC_RELEASE_DIR'
  exit 1
fi

QGC_SRC=$(readlink -f $1)

QGC_CUSTOM_APP_NAME="${QGC_CUSTOM_APP_NAME:-MissionControl}"
QGC_CUSTOM_GENERIC_NAME="${QGC_CUSTOM_GENERIC_NAME:-Ground Control Station}"
QGC_CUSTOM_BINARY_NAME="${QGC_CUSTOM_BINARY_NAME:-MissionControl}"
QGC_CUSTOM_LINUX_START_SH="${QGC_CUSTOM_LINUX_START_SH:-${QGC_SRC}/deploy/missioncontrol-start.sh}"
QGC_CUSTOM_APP_ICON="${QGC_CUSTOM_APP_ICON:-${QGC_SRC}/resources/icons/missioncontrol.png}"
QGC_CUSTOM_APP_ICON_NAME="${QGC_CUSTOM_APP_ICON_NAME:-MissionControl}"
...
# copy MissionControl release into appimage
...
cat > ./missioncontrol.desktop <<\EOF
[Desktop Entry]
Type=Application
